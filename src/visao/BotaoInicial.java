...
/**
 * Um botão personalizado com efeitos visuais.
 */
public class BotaoInicial extends JButton {
  ...
  private Color cor1 = Color.decode("#020054");
  private Color cor2 = Color.decode("#001aa0");
  private final Timer timer;
  private final Timer timerPressionado;
  ...
  /**
   * Constrói um novo BotaoInicial com o texto especificado.
   *
   * @param text o texto a ser exibido no botão
   */
  public BotaoInicial(String text) {
    setText(text);
    ...
    timerPressionado = new Timer(0, new ActionListener() {
      @Override
      public void actionPerformed(ActionEvent ae) {
        pressedSize += getSizeSpeed();
        if (alphaPressed <= 0) {
          pressed = false;
          timerPressionado.stop();
        } else {
          repaint();
        }
      }
  ...
  protected void paintComponent(Graphics grphcs) {
    ...
    GradientPaint gra = new GradientPaint(0, 0, cor1, width, 0, cor2);
    g2.setPaint(gra);
  ...
  protected void createStyle(Graphics2D g2) {
    g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, alpha));
    int width = getWidth();
    int height = getHeight();
    GradientPaint gra =
        new GradientPaint(0, 0, Color.WHITE, 0, height, new Color(255, 255, 255, 60));
    g2.setPaint(gra);
    Path2D.Float f = new Path2D.Float();
    f.moveTo(0, 0);
    int controll = height + height / 2;
    f.curveTo(0, 0, width / 2.0, controll, width, 0);
    g2.fill(f);
  }
  ...
  private void adicionarMouseListener() {
    addMouseListener(new MouseAdapter() {
      ...
      public void mousePressed(MouseEvent me) {
        ...
        timerPressionado.setDelay(0);
        timerPressionado.start();
      }
    });
  }
